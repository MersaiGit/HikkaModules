...
class MegaMozgMod(loader.Module):
    ...
    async def mozgchancecmd(self, m: types.Message):
        ".mozgchance  - Установить шанс 1 к X.\n0 - всегда отвечать"
    ...
    async def setypecmd(self, m: types.Message):
        ".setype  - Установить время печатания перед ответом"
        args: str = utils.get_args_raw(m)
        if args.isdigit():
            self.db.set("MozgType", "typing_time", int(args))
            return await utils.answer(
                m, "[MozgType] {}".format(args)
            )
    ...
    async def watcher(self, m: types.Message):
        ...
        typing_time = self.db.get("MozgType", "typing_time", 3)
        async with m.client.action(m.chat_id, "typing"):
